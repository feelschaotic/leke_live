 ...
 */
public class Type2Dialog extends Dialog {
    public static Map<String,Integer> valMap=new HashMap<>();
    public Type2Dialog(Context context) {
        super(context);
    }
    public Type2Dialog(Context context, int theme) {
        super(context, theme);
    }
...
//            adapter = new DialogAdapter(context, List);
        ...
        public Type2Dialog create() {
            LayoutInflater inflater = (LayoutInflater) context
                    .getSystemService(Context.LAYOUT_INFLATER_SERVICE);

            final Type2Dialog dialog = new Type2Dialog(context, R.style.Dialog);
//            View layout = inflater.inflate(R.layout.dialog_type, null);
            ...
            dialog_lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    ...
                    L.e(name);
                    listener.getName(valMap.get(name),name);
                }
            });
           ...
           /* for (int i = 0; i < 10; i++) {
            ...
            NetUtil.getHttpUrlConnection(ServerConstants.TypeListURL, new HttpCallbackListener() {
                ...
                public void onSucc(String response) {
                    Gson gson = new Gson();
                    typeList = gson.fromJson(response, new TypeToken<List<Type>>() {
                    }.getType());
                    ...
                    for (Type p : typeList) {
                        ...
                        mapList.add(map);
                        valMap.put(p.getTypeName(), p.getTypeId());
                    }
                    handler.sendEmptyMessage(1);
                }
