...
import com.ramo.campuslive.adapter.EuclidState;
 ...
 */
public abstract class EuclidActivity extends SwipeBackActivity {
    ...
    protected int nowLivePos = 0;

    private void initList() {
        BaseAdapter adapter = getAdapter();
        mListViewAnimationAdapter = new SwingLeftInAnimationAdapter(adapter);
    ...
    private void addOverlayListItem(Map<String, Object> item, View view) {
        if (mOverlayListItemView == null) {
            mOverlayListItemView = getLayoutInflater().inflate(com.yalantis.euclid.library.R.layout.overlay_list_item, mWrapper, false);
        } else {
...
*/
        ImageView iv1 = (ImageView) mOverlayListItemView.findViewById(com.yalantis.euclid.library.R.id.image_view_reveal_avatar);
        ImageView iv2 = (ImageView) mOverlayListItemView.findViewById(com.yalantis.euclid.library.R.id.image_view_avatar);
        L.e(EuclidListAdapter.mImageLoader + "!!@");
       /* iv1.setImageResource(R.drawable.head);
        iv2.setImageResource(R.drawable.head);*/
        String url = ServerConstants.ProUrl + item.get(EuclidListAdapter.KEY_AVATAR);
        EuclidListAdapter.mImageLoader.loaderImageByThread(iv1, url);
        // EuclidListAdapter.mImageLoader.loaderImageByThread(iv2, url);
        iv1.setTag(url);
        iv2.setTag(url);
    ...
    private void createOpenProfileButtonAnimation() {
        if (mProfileButtonShowAnimation == null) {
            mProfileButtonShowAnimation = AnimationUtils.loadAnimation(this, com.yalantis.euclid.library.R.anim.profile_button_scale);
